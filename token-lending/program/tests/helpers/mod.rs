...
pub struct TestStakingPool {
    ...
    pub staking_pool: port_finance_staking::state::staking_pool::StakingPool,
}
...
impl TestStakingPool {
    ...
    ) -> port_finance_staking::state::staking_pool::StakingPool {
        let staking_pool: Account = banks_client
            .get_account(self.staking_pool_pubkey)
            .await
            .unwrap()
            .unwrap();
        port_finance_staking::state::staking_pool::StakingPool::unpack(&staking_pool.data[..])
            .unwrap()
    }
}
...
pub fn add_staking_pool(
    ...
    staking_pool_owner_pubkey: Pubkey,
    total_deposit: u64,
) -> TestStakingPool {
    ...
    let mut staking_pool = port_finance_staking::state::staking_pool::StakingPool::default();
    ...
    staking_pool.owner_authority = staking_pool_owner_pubkey;
    staking_pool.duration = 1000;
    staking_pool.rate_per_slot = RatePerSlot {
        reward: port_finance_staking::math::Decimal::one(),
        sub_reward: None,
    };
    staking_pool.pool_size = total_deposit;

    staking_pool
        .init(
            staking_pool_owner_pubkey,
            Pubkey::new_unique(),
            reward_token_pool_pubkey,
            None,
            1000,
...
#[derive(Debug)]
pub struct TestStakeAccount<'a> {
    pub name: String,
    pub pubkey: Pubkey,
    pub owner: &'a Keypair,
    pub stake_account: port_finance_staking::state::stake_account::StakeAccount,
}

impl<'a> TestStakeAccount<'a> {
    pub async fn get_state(
        &self,
        banks_client: &mut BanksClient,
    ) -> port_finance_staking::state::stake_account::StakeAccount {
        let staking_pool: Account = banks_client
            .get_account(self.pubkey)
            .await
            .unwrap()
            .unwrap();
        port_finance_staking::state::stake_account::StakeAccount::unpack(&staking_pool.data[..])
            .unwrap()
    }
}

pub fn add_stake_account<'a, 'b>(
    test: &'b mut ProgramTest,
    staking_pool: Pubkey,
    owner: &'a Keypair,
    deposited_value: u64,
) -> TestStakeAccount<'a> {
    let stake_account_pubkey = Pubkey::new_unique();
    let mut stake_account = port_finance_staking::state::stake_account::StakeAccount::default();
    stake_account.deposited_amount = deposited_value;
    ...
    TestStakeAccount {
        name: "Port Finance Staking account".to_owned(),
...
pub fn add_obligation(
    ...
    let (obligation_borrows, test_borrows) = borrows
        ...
        .map(|(borrow_reserve, liquidity_amount)| {
            ...
            liquidity.cumulative_borrow_rate_wads = Decimal::one();
...
pub fn add_reserve(
    ...
    let mut reserve = Reserve::new(InitReserveParams {
        ...
        liquidity: ReserveLiquidity::new(NewReserveLiquidityParams {
            ...
            oracle_pubkey: COption::Some(oracle.price_pubkey),
    ...
    TestReserve {
        ...
        liquidity_fee_receiver_pubkey,
        liquidity_host_pubkey,
        liquidity_oracle_pubkey: COption::Some(oracle.price_pubkey),
        collateral_mint_pubkey,
        collateral_supply_pubkey,
        user_liquidity_pubkey,
        user_collateral_pubkey,
...
pub struct LiquidateArgs<'a> {
    ...
    pub user_accounts_owner: &'a Keypair,
    pub borrow_stake_account: Option<Pubkey>,
    pub borrow_staking_pool: Option<Pubkey>,
}
...
pub struct TestReserve {
    ...
    pub liquidity_oracle_pubkey: COption<Pubkey>,
...
impl TestReserve {
    ...
    pub async fn init(
        ...
        let rent = banks_client.get_rent().await.unwrap();
        let price_oracle = if oracle.is_some() {
            COption::Some(oracle.clone().unwrap().price_pubkey)
        } else {
            COption::None
        };
        let mut transaction = Transaction::new_with_payer(
        ...
        banks_client
            ...
            .map(|_| Self {
                ...
                liquidity_oracle_pubkey: price_oracle,
                ...
                market_price: if oracle.is_some() {
                    oracle.unwrap().price
                } else {
                    fixed_price.unwrap()
                },
    ...
    pub async fn validate_state(&self, banks_client: &mut BanksClient) {
        ...
        assert_eq!(self.market_price, reserve.liquidity.market_price,);
...
pub fn add_sol_pyth_oracle(test: &mut ProgramTest) -> TestOracle {
    add_pyth_oracle(
        test,
...
pub fn add_usdc_pyth_oracle(test: &mut ProgramTest) -> TestOracle {
    add_pyth_oracle(
        test,
